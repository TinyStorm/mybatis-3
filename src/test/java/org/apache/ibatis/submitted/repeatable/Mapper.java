...
public interface Mapper {
  ...
  @Insert(value = "INSERT INTO users (id, name) VALUES(#{user.id}, #{user.name} || ' HSQL')", databaseId = "hsql")
  @Insert(value = "INSERT INTO users (id, name) VALUES(#{user.id}, #{user.name} || ' DERBY')", databaseId = "derby")
  @Insert("INSERT INTO users (id, name) VALUES(#{id}, #{user.name} || ' DEFAULT')")
  @SelectKey(statement = "SELECT COUNT(*) + 1 FROM users", keyProperty = "user.id", before = true, resultType = Integer.class, databaseId = "hsql")
  @SelectKey(statement = "SELECT COUNT(*) + 1001 FROM users", keyProperty = "user.id", before = true, resultType = Integer.class, databaseId = "derby")
  @SelectKey(statement = "SELECT COUNT(*) + 10001 FROM users", keyProperty = "user.id", before = true, resultType = Integer.class)
  void insertNamedUser(@Param("user") User user);
